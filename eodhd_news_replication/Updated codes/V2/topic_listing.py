...
Step 3:
    ...
    - Apply structural + finance/ticker garbage filtering + chi2 feature selection
...
INPUT_PARQUET_PATH = "/Users/vivan/Desktop/Central File Manager/02 USC/04 Semester 3/03 DSO 585 - Data Driven Consulting/01 Project/news-event-study-stocks/mag7_news_with_topics_sentimentV2.parquet"
OUTPUT_PARQUET_PATH = "mag7_news_with_sentiment_and_topics_labeledV2.parquet"
TOPIC_TERMS_JSON_PATH = "topic_terms_kmeansV2.json"
TOPIC_LABELS_JSON_PATH = "topic_labels_kmeansV2.json"
...
# Very short tokens we *do* keep
SHORT_WHITELIST = {"ai", "fx", "eu", "us", "uk", "fed"}

# Finance words that are noise as topic descriptors
GENERIC_FIN_STOP = {
...
# Company / ticker-like words you said you don't want as topic labels
TICKER_STOP = {
    "tesla", "nvidia", "apple", "amazon", "microsoft", "meta",
    "alphabet", "google", "goog", "googl",
    "palantir", "broadcom",
    # add more here as you see junk in topic_terms
}

# Important people you explicitly want to *keep*
PERSON_WHITELIST = {"trump", "musk"}


def is_garbage_term(term: str) -> bool:
    """
    Generic structural/regex + domain rules to filter out junk terms
    when building topic descriptors.
    """
    t = term.lower().strip()

    # ALWAYS keep certain people even if they look like other junk
    if t in PERSON_WHITELIST:
        return False
    ...
    # If ANY token in the phrase is a generic finance stopword
    if any(tok in GENERIC_FIN_STOP for tok in tokens):
        return True

    # If ANY token is a ticker/company you want to ignore
    if any(tok in TICKER_STOP for tok in tokens):
        return True

    # All-caps ticker patterns like AAPL, NVDA, TSLA, GOOG, etc.
    if re.fullmatch(r"[A-Z]{2,6}(\.[A-Z]{1,3})?", term):
        return True
...
def describe_topics_with_words(df_emb,
    ...
    For each topic_id, return:
      - top_n words/phrases (by mean TF-IDF) AFTER
        * structural & domain garbage filtering
    ...
    for t_id in unique_topics:
        idx = df_emb.index[df_emb[topic_col] == t_id]
        if len(idx) == 0:
            continue
        ...
        topic_terms[int(t_id)] = top_terms
        ...
        topic_labels[int(t_id)] = " / ".join(label_terms)

        print(f"\n=== Topic {t_id} ===")
        print("Top terms:", ", ".join(top_terms))
        print("Auto label:", topic_labels[int(t_id)])
