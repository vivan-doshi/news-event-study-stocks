...
def main():
    ...
    news_dir = "reports/fama_french_news/data"
    output_report_path = "reports/fama_french_news/news_analysis_summary.md"
    ...
    report_lines = []
    report_lines.append("# News Augmented Analysis Results")
    report_lines.append("Comparing Baseline (Fama-French + Lags) vs. News Augmented (FF + Lags + Total News Logs)")
    report_lines.append("")
    ...
    for sym in symbols:
        ...
        summary_table.append(f"| {sym} | {base_r2:.4f} | {news_r2:.4f} | {diff_r2:+.4f} | {base_rmse:.5f} | {news_rmse:.5f} | {improved} |")
    ...
    report_lines.append("## Detailed Results by Stock")
    
    for sym in symbols:
        report_lines.append(f"### {sym}")
        report_lines.append(f"![{sym} With News]({os.path.abspath(f'reports/fama_french_news/plots/{sym}_news_augmented.png')})")
        report_lines.append("")
