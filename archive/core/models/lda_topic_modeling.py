...
class LDATopicModeling:
    ...
    def load_data(self) -> Tuple[csr_matrix, pd.DataFrame, pd.DataFrame]:
        ...
        dtm = load_npz("data/processed/dtm.npz")
        doc_index = pd.read_parquet("data/processed/doc_index.parquet")
        term_index = pd.read_parquet("data/processed/term_index.parquet")
    ...
    def hyperparameter_search(self, dtm: csr_matrix) -> pd.DataFrame:
        ...
        # Save results
        results_df.to_csv("data/outputs/models/lda_cv_results.csv", index=False)
    ...
    def train_final_model(self, dtm: csr_matrix, doc_index: pd.DataFrame,
        ...
        model_dir = Path(f"data/outputs/models/lda_k={n_topics:03d}")
...
def run_lda_pipeline(n_topics: Optional[int] = None) -> Dict:
    ...
    # Save online results
    online_dir = Path(f"data/outputs/models/lda_k={n_topics:03d}_online")
    ...
    with open(f"data/outputs/models/lda_k={n_topics:03d}/topic_labels.json", 'w') as f:
