...
class TopicTimeAggregation:
    ...
    def load_topic_data(self, model_name: str = None, online: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if model_name is None:
            model_dirs = list(Path("data/outputs/models").glob("lda_k=*"))
            if not model_dirs:
        ...
        if online:
            theta_file = Path(f"data/outputs/models/{model_name}_online/theta_online.parquet")
        else:
            theta_file = Path(f"data/outputs/models/{model_name}/theta.parquet")
        ...
        # Load document index
        doc_index = pd.read_parquet("data/processed/doc_index.parquet")

        # Load clean data for document lengths
        clean_df = pd.read_parquet("data/processed/news_clean.parquet")
    ...
    def merge_with_returns(self, topic_series: pd.DataFrame, frequency: str = 'monthly') -> pd.DataFrame:
        ...
        # Load returns data
        if frequency == 'daily':
            returns_file = "data/processed/returns_daily.parquet"
        else:
            returns_file = "data/processed/returns_monthly.parquet"
            ...
            try:
                ...
                for frequency in self.frequencies:
                    for method in self.methods:
                        ...
                        # Save series
                        output_file = f"data/processed/topics_{frequency}_{method}_{model_type}.parquet"
                        series.to_parquet(output_file, compression='snappy')
                        logger.info(f"Saved {output_file}")

                        # Save statistics
                        stats_file = f"data/processed/topics_{frequency}_{method}_{model_type}_stats.csv"
                        stats.to_csv(stats_file)

                        # Merge with returns and create panel
                        panel = self.merge_with_returns(series, frequency)
                        panel_file = f"data/processed/panel_{frequency}_{method}_{model_type}.parquet"
...
def run_time_aggregation_pipeline(model_name: str = None) -> Dict:
    ...
    if 'monthly_mean_batch' in all_series:
        ...
        if not spikes.empty:
            spikes.to_csv("data/outputs/results/topic_spikes.csv", index=False)
        ...
        aggregator.export_for_visualization(
            monthly_series,
            "data/outputs/results/topic_attention_monthly.csv"
        )
